...
const imageBaseUrl = `https://gitee.com/${owner}/${imgs_repo}/raw/master/${image_folder}/`;
...
async function uploadImage(fileContent, filename) {
  const apiUrl = `https://gitee.com/api/v5/repos/${owner}/${imgs_repo}/contents/${image_folder}/${filename}`;
...
async function fetchImage(uniqueFilename) {
  if (!uniqueFilename) {
    console.error("Missing filename for image fetch");
    return null;
  }
  
  try {
    const imagePath = `https://gitee.com/api/v5/repos/${owner}/${imgs_repo}/contents/${image_folder}/${uniqueFilename}`;
    ...
    console.error('Error in fetchImage:', error, "for file:", uniqueFilename);
    return null;
  }
}
